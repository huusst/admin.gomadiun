...
const TableAdmin = () => {
    ...
    const getData = async (searchTerm = '') => {
        ...
        try {
            const url = searchTerm
                ? `${process.env.REACT_APP_BACKEND_API_URL}/api/adminIndustri/get_all?keyword=${searchTerm}`
                : `${process.env.REACT_APP_BACKEND_API_URL}/api/adminIndustri/get_all`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminIndustri/add_data`, formDataObj)
            if (response) {
    ...
    const handleSubmitUpdate = async (e) => {
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminIndustri/update/${formDataEdit.id}`, formDataObj)
            if (response) {
                setResponseMessage(response.data.message);
                setResponseMessageStatus(response.data.status);
                closeModalEdit();
                    ...
                    <div className="col-lg-12 grid-margin stretch-card mt-3">
                        <div className="card">
                            <div className="card-body">
                                <h4 className="card-title">Tabel Admin Industri</h4>
                                {responseMessage && (
