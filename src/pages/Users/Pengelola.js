...
const TableAdmin = () => {
    ...
    const getData = async (searchTerm = '') => {
        ...
        try {
            const url = searchTerm
                ? `${process.env.REACT_APP_BACKEND_API_URL}/api/adminPengelola/get_all?keyword=${searchTerm}`
                : `${process.env.REACT_APP_BACKEND_API_URL}/api/adminPengelola/get_all`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminPengelola/add_data`, formDataObj)
            if (response) {
    ...
    const handleSubmitUpdate = async (e) => {
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminPengelola/update/${formDataEdit.id}`, formDataObj)
            if (response) {
                setResponseMessage(response.data.message);
                setResponseMessageStatus(response.data.status);
                closeModalEdit();
