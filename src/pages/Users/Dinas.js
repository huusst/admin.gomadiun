...
const TableAdmin = () => {
    ...
    const getData = async (searchTerm = '') => {
        ...
        try {
            const url = searchTerm
                ? `${process.env.REACT_APP_BACKEND_API_URL}/api/adminDinas/get_all?keyword=${searchTerm}`
                : `${process.env.REACT_APP_BACKEND_API_URL}/api/adminDinas/get_all`;
    ...
    const openModalEdit = (id_admin, name_admin, email, nama_lengkap, nip, status_akun, sampul) => {
        setFormDataEdit({
            id: id_admin,
            name: name_admin,
            email: email,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminDinas/add_data`, formDataObj)
            if (response) {
    ...
    const handleSubmitUpdate = async (e) => {
        ...
        formDataObj.append("nip", formDataEdit.nip);
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_API_URL}/api/adminDinas/update/${formDataEdit.id}`, formDataObj)
            if (response) {
                setResponseMessage(response.data.message);
                setResponseMessageStatus(response.data.status);
                closeModalEdit();
                            ...
                            <div className="modal-content slideDown">
                                ...
                                <div className="modal-body">
                                    <form onSubmit={handleSubmit} className="modal-form">
                                        ...
                                        <div className="form-group">
                                            <label htmlFor="nip">NIP Admin</label>
                                            <input
                                                type="text"
                                                name="nip"
                                                value={formData.nip}
                    ...
                    <div className="col-lg-12 grid-margin stretch-card mt-3">
                        <div className="card">
                            <div className="card-body">
                                <h4 className="card-title">Tabel Admin Dinas</h4>
                                {responseMessage && (
                                                ...
                                                <>
                                                    {currentPageData.map((item, index) => (
                                                        <tr key={index} className='under-line'>
                                                            ...
                                                            <td><FontAwesomeIcon icon={faEdit} onClick={() => openModalEdit(item.id_admin, item.nama_admin, item.email_admin, item.namaLengkap_admin, item.nip_admin, item.status_akun, item.sampul_admin)} />
